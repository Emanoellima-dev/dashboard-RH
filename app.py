...
app.layout = dbc.Container([
    dbc.Row([
      dbc.Col([
         html.H1("Dashboard De RH", style={"text-align": "center"}),
...
def update_graph(value, toggle):
  ...
  if value == "Cidade":
      fig1 = px.bar(func_por_cidade, x="Cidade", y="qtde de Funcionarios", title="Funcionarios por cidade", template=template)
    ...
    fig2 = px.bar(func_por_area, x="Area", y="qtde de Funcionarios", title="funcionarios por area", template=template)
    return fig2
    
  else:
    fig3 = px.bar(func_por_cargo, x="Cargo", y="qtde de Funcionarios", title="funcionarios por cargo", template=template)
    return fig3
